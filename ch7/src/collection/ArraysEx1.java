...
public class ArraysEx1 {
	public static void main(String[] args) {
		...
		//fill() :  배열의 모든 요소를 저장한 값으로 채우기
		int arr7[] = new int[5];
		Arrays.fill(arr7, 5); //void가 리턴일땐 메소드 호출만 함 
		...
		//검색
		//순차검색 - 차례대로 검색하는 방식
		//이진검색 - 검색할 범위를 반복적으로 절반씩 줄여나가면서 검색하는 방식 - 정렬이 먼저 되어있어야함 
		//찾게되면 요소의 위치를 돌려줌 
		System.out.println(Arrays.binarySearch(arr8, 4));
		System.out.println(Arrays.binarySearch(arr8, 5));
		
		//asList(T... a) : 배열을 List로 변환 (크기변경은 불가능함)
		// T... : 가변인수
		// T : 기본타입이 아니어야함 
		List<Integer> list2 = Arrays.asList(1,2,3,4,7,8);
		// List<Integer> list = Arrays.asList(arr8); => arr8이 기본타입인 int라서 안됨
		
		Integer[] arr9 = {15,16,17,18};
		List<Integer> list = Arrays.asList(arr9); //arr9는 참조타입이라서 가능 
		
		//list크기변경을 원할땐 list.add()로 못하고
